        ...
        callApi(
            ...
            .then(listLease => {
                if (listLease instanceof Array) {
                    const data = listLease.map(lease => {
                        let amountSale = lease.product.cost
                        let { events } = lease.product
                    ...
                    this.setState({ listLease: data }, () => console.log('setState'))
                }
            })
            ...
            },
            {
                ...
                render: record => {
                    ...
                    if (curr.isBefore(start))
                        return <Tag color="#108ee9">Đang chờ thuê</Tag>
            ...
            },
            {
                ...
                render: record => {
                    ...
                    let duration = moment.duration(
                        moment(startTime).diff(this.state.currentTime)
                    )
                    if (duration < 0)
                        duration = moment.duration(
                            moment(endTime).diff(this.state.currentTime)
                        )
                    return `${Math.floor(
                        duration.asDays()
                    )} ngày ${duration.hours()} giờ ${duration.minutes()} phút ${duration.seconds()} giây`
                },
            },
        ]
        const dataSource = this.state.listLease
            .filter(lease => moment(Number(lease.endTime)) > moment())
            .map(lease => ({
                key: lease.id,
