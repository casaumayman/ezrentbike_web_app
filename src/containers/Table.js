...
class Table extends Component {
    ...
    onSortData = field => {
        ...
        if (
            typeof this.state.data.rows[0][field] !== 'string' &&
            typeof this.state.data.rows[0][field] !== 'number'
        )
            return
        if (
            this.state.sort.substr(0, fields.length - 1) === field &&
            ...
            this.setState({
                data: { ...this.props.data, rows: rows },
                sort: `${field}1`,
            })
        }
    }
    render() {
        ...
        const rows = newRows.slice(
            ...
            Math.min((currentPage - 1) * pageSize + pageSize, newRows.length)
        )
        for (let i = 0; i < maxPage; i++) {
            pagination.push(
                <li
                    ...
                    className={
                        `page-item ` +
                        (this.state.currentPage === i + 1 ? 'active' : '')
                    }
                                ...
                                <tr>
                                    {columns.map((value, index) => (
                                        <th
                                            ...
                                            scope="col"
                                            onClick={() =>
                                                this.onSortData(value.field)
                                            }
                                        >
                                            {value.label}
                                            {value.field !== 'action' ? (
                                                this.state.sort.substr(
                                                    0,
                                                    this.state.sort.length - 1
                                                ) !== value.field ? (
                                                    <MDBIcon
                                                        icon="sort"
                                                        style={{
